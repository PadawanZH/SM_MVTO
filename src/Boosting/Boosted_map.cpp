...
bool Boosted_map::tm_insert (long key, void* data) {
    ...
    debuglog(LogLevel::INFO, "Tx [%ld] INSERT with key %ld",BaseTransaction::currentTXID, key);
    if(result){
        BaseTransaction::undoLog.push_back([this, key]{
            debuglog(LogLevel::WARN, "Tx [%ld] undo INSERT action by remove key %ld",BaseTransaction::currentTXID, key);
...
std::pair<bool, std::pair<long, void*>> Boosted_map::tm_remove (long key) {
    ...
    if(result){//find it! delete it
        ...
        debuglog(LogLevel::INFO, "Tx [%ld] REMOVE with key %ld",BaseTransaction::currentTXID, key);
        BaseTransaction::undoLog.push_back([this, key, data](){
            debuglog(LogLevel::WARN, "Tx [%ld] undo REMOVE action by insert key %ld",BaseTransaction::currentTXID, key);
        ...
        debuglog(LogLevel::INFO, "Tx [%ld] want to REMOVE with key %ld, but NOT FOUND IN MAP",BaseTransaction::currentTXID, key);
...
bool Boosted_map::tm_contains(long key) {
    ...
    tbb::concurrent_hash_map<long, void*>::const_accessor reader;
    bool result = hash_map.find(reader, key);
    debuglog(LogLevel::INFO, "Tx [%ld] check CONTAINS with key %ld, and result is %s",BaseTransaction::currentTXID, key, result? (char*)"true" : (char*)"false");
    return result;
...
void* Boosted_map::tm_find(long key) {
    ...
    tbb::concurrent_hash_map<long, void*>::const_accessor reader;
    bool result = hash_map.find(reader, key);
    debuglog(LogLevel::INFO, "Tx [%ld] FIND with key %ld, and result is %s",BaseTransaction::currentTXID, key, result? (char*)"true" : (char*)"false");
    if(result){
